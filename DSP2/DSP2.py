...
canny_operator_image_array = fp.canny_operator(image_array, 125, 75)
...
for x in range(width):
	for y in range(height):
		...
		product = r * g * b // (255 * 255)
		# Set the pixel value in the result image
		processed_image.putpixel((x, y), product)

processed_image_array = grayscale(image_array)
processed_image_array = cv2.Canny(processed_image_array, 90, 125)

#processed_image_array = solarize((high_pass_filter(high_pass_filter(negative(image_array))))).astype(np.uint8)
